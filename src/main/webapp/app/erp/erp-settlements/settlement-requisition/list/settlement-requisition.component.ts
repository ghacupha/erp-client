...
import { EntityArrayResponseType, SettlementRequisitionService } from '../service/settlement-requisition.service';
...
@Component({
  ...
  templateUrl: './settlement-requisition.component.html',
})
export class SettlementRequisitionComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['description', 'serialNumber', 'requisitionNumber', 'paymentStatus'];

  settlementRequisitions?: ISettlementRequisition[];
  isLoading = false;
  ...
  trackId = (_index: number, item: ISettlementRequisition): number =>
    this.settlementRequisitionService.getSettlementRequisitionIdentifier(item);

  search(query: string): void {
    if (query && SettlementRequisitionComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (SettlementRequisitionComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.settlementRequisitions = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISettlementRequisition[] | null): ISettlementRequisition[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.settlementRequisitionService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.settlementRequisitionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
