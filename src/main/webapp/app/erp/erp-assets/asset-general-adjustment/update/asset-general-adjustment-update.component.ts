...
export class AssetGeneralAdjustmentUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ assetGeneralAdjustment }) => {
      if (assetGeneralAdjustment.id === undefined) {
        ...
        assetGeneralAdjustment.adjustmentReferenceId = uuidv7();
      }
  ...
  updateAssetRegistration(updated: IAssetRegistration): void {
    this.editForm.patchValue({ assetRegistration: updated });
  }
