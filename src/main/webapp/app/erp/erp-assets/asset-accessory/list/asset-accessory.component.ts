...
export class AssetAccessoryComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.assetAccessoryService
        ...
        .subscribe(
          (res: HttpResponse<IAssetAccessory[]>) => {
  ...
  search(query: string): void {
    if (query && ['assetTag', 'assetDetails', 'comments', 'modelNumber', 'serialNumber'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: IAssetAccessory): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IAssetAccessory[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/asset-accessory'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
