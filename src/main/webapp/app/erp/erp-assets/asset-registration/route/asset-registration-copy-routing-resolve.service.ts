...
import { assetRegistrationCopyWorkflowInitiatedEnRoute } from '../../../store/actions/fixed-assets-register-update-status.actions';

@Injectable({ providedIn: 'root' })
export class AssetRegistrationCopyRoutingResolveService implements Resolve<IAssetRegistration> {
      ...
      return this.service.find(id).pipe(
        mergeMap((assetRegistration: HttpResponse<AssetRegistration>) => {
          if (assetRegistration.body) {
            this.store.dispatch(assetRegistrationCopyWorkflowInitiatedEnRoute({ copiedInstance: assetRegistration.body }));
