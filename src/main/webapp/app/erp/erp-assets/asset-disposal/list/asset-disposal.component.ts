...
export class AssetDisposalComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.assetDisposalService
        ...
        .subscribe(
          (res: HttpResponse<IAssetDisposal[]>) => {
  ...
  search(query: string): void {
    if (query && ['assetDisposalReference', 'description'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: IAssetDisposal): number {
    return item.id!;
  }

  delete(assetDisposal: IAssetDisposal): void {
    const modalRef = this.modalService.open(AssetDisposalDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.assetDisposal = assetDisposal;
  ...
  protected onSuccess(data: IAssetDisposal[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
