...
describe('DepreciationMethod Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    ...
    it('Should update editForm', () => {
      const depreciationMethod: IDepreciationMethod = { id: 456 };
      const placeholders: IPlaceholder = { id: 21821 };
      depreciationMethod.placeholders = [placeholders];

      activatedRoute.data = of({ depreciationMethod });
      comp.ngOnInit();

      expect(comp.editForm.value).toEqual(expect.objectContaining(depreciationMethod));
  ...
  describe('save', () => {
    it('Should call update service on save for existing entity', () => {
      // GIVEN
      const saveSubject = new Subject<HttpResponse<DepreciationMethod>>();
      ...
      expect(depreciationMethodService.update).toHaveBeenCalledWith(depreciationMethod);
    ...
    it('Should call create service on save for new entity', () => {
      // GIVEN
      const saveSubject = new Subject<HttpResponse<DepreciationMethod>>();
      const depreciationMethod = new DepreciationMethod();
      ...
      // THEN
      expect(depreciationMethodService.create).toHaveBeenCalledWith(depreciationMethod);
      expect(comp.isSaving).toEqual(false);
      expect(comp.previousState).toHaveBeenCalled();
    });
