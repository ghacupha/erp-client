 ...
 */
@Component({
  selector: 'jhi-m21-fiscal-month-form-control',
  templateUrl: './m21-fiscal-month-form-control.component.html',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => M21FiscalMonthFormControlComponent),
      multi: true
    }
  ]
})
export class M21FiscalMonthFormControlComponent implements OnInit, ControlValueAccessor, OnDestroy {

  @Input() inputValue: IFiscalMonth = {}
  ...
  valueLookUps$: Observable<IFiscalMonth[]> = of([]);

  constructor(
    protected valueSuggestionService: FiscalMonthSuggestionService
  ) {}
  ...
  writeValue(value: IFiscalMonth): void {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    if (value) {
