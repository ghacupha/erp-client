...
@Component({
  selector: 'jhi-m2m-wip-registration-form-control',
  templateUrl: './m2m-wip-registration-form-control.component.html',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => M2mWipRegistrationFormControlComponent),
      multi: true
    }
  ]
})
export class M2mWipRegistrationFormControlComponent implements OnInit, OnDestroy, ControlValueAccessor {

  @Input() inputValues: IWorkInProgressRegistration[] = [];

  @Input() inputControlLabel = '';

  @Output() valueSelected: EventEmitter<IWorkInProgressRegistration[]> = new EventEmitter<IWorkInProgressRegistration[]>();
   ...
   * @param value
   */
  writeValue(value: IWorkInProgressRegistration[]): void {
    if (value.length !== 0) {
