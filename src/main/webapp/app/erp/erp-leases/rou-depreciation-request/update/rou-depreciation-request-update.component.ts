  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ rouDepreciationRequest }) => {
      if (rouDepreciationRequest.id === undefined) {
        ...
        rouDepreciationRequest.timeOfRequest = dayjs();
        rouDepreciationRequest.requisitionId = uuidv7();
        rouDepreciationRequest.depreciationProcessStatus = depreciationProcessStatusTypes.STARTED;
      }
