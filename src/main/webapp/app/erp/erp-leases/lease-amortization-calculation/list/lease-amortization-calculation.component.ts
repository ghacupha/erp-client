...
@Component({
  ...
  templateUrl: './lease-amortization-calculation.component.html',
})
export class LeaseAmortizationCalculationComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.leaseAmortizationCalculationService
        ...
        .subscribe(
          (res: HttpResponse<ILeaseAmortizationCalculation[]>) => {
  ...
  search(query: string): void {
    if (query && ['periodicity'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: ILeaseAmortizationCalculation): number {
    return item.id!;
  }

  delete(leaseAmortizationCalculation: ILeaseAmortizationCalculation): void {
    const modalRef = this.modalService.open(LeaseAmortizationCalculationDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.leaseAmortizationCalculation = leaseAmortizationCalculation;
  ...
  protected onSuccess(data: ILeaseAmortizationCalculation[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/lease-amortization-calculation'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.leaseAmortizationCalculations = data ?? [];
    this.ngbPaginationPage = this.page;
  }
