...
export class PdfReportRequisitionComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.pdfReportRequisitionService
        .search({
          page: pageToLoad - 1,
        ...
        .subscribe(
          (res: HttpResponse<IPdfReportRequisition[]>) => {
  ...
  protected onSuccess(data: IPdfReportRequisition[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/pdf-report-requisition'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.pdfReportRequisitions = data ?? [];
    this.ngbPaginationPage = this.page;
  }
