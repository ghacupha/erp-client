...
export class PrepaymentOutstandingOverviewReportService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/prepayments/prepayment-outstanding-overview-reports');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/prepayments/_search/prepayment-outstanding-overview-reports');
  ...
  findByDate(reportDate: string): Observable<EntityResponseType> {
    ...
    return this.http.get<IPrepaymentOutstandingOverviewReport>(`${this.resourceUrl}`, { params: reportParams, observe: 'response' });
  }
