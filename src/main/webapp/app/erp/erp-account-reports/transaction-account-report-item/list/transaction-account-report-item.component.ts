...
@Component({
  ...
  templateUrl: './transaction-account-report-item.component.html',
})
export class TransactionAccountReportItemComponent implements OnInit {
  transactionAccountReportItems?: ITransactionAccountReportItem[];
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.transactionAccountReportItemService
        ...
        .subscribe(
          (res: HttpResponse<ITransactionAccountReportItem[]>) => {
  ...
  trackId(index: number, item: ITransactionAccountReportItem): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ITransactionAccountReportItem[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/transaction-account-report-item'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.transactionAccountReportItems = data ?? [];
    this.ngbPaginationPage = this.page;
  }
