...
export class FraudCategoryFlagComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.fraudCategoryFlagService
        ...
        .subscribe(
          (res: HttpResponse<IFraudCategoryFlag[]>) => {
  ...
  search(query: string): void {
    if (query && ['fraudCategoryFlag', 'fraudCategoryTypeDetails'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: IFraudCategoryFlag): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IFraudCategoryFlag[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/fraud-category-flag'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
