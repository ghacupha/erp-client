...
export class CategoryOfSecurityComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.categoryOfSecurityService
        .search({
          page: pageToLoad - 1,
        ...
        .subscribe(
          (res: HttpResponse<ICategoryOfSecurity[]>) => {
  ...
  search(query: string): void {
    if (query && ['categoryOfSecurity', 'categoryOfSecurityDetails', 'categoryOfSecurityDescription'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: ICategoryOfSecurity): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ICategoryOfSecurity[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/category-of-security'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.categoryOfSecurities = data ?? [];
    this.ngbPaginationPage = this.page;
  }
