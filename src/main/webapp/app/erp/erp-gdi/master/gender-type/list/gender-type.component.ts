...
export class GenderTypeComponent implements OnInit {
  ...
  search(query: string): void {
    if (query && ['genderCode', 'genderType', 'genderDescription'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: IGenderType): number {
    return item.id!;
  }
  ...
  delete(genderType: IGenderType): void {
    const modalRef = this.modalService.open(GenderTypeDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IGenderType[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
