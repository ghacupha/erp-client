...
export class CardTypesComponent implements OnInit {
  ...
  search(query: string): void {
    if (query && ['cardTypeCode', 'cardType', 'cardTypeDetails'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  protected onSuccess(data: ICardTypes[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/card-types'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
