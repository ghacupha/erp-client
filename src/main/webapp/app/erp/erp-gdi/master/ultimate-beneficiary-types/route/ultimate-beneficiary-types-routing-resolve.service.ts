...
export class UltimateBeneficiaryTypesRoutingResolveService implements Resolve<IUltimateBeneficiaryTypes> {
  constructor(protected service: UltimateBeneficiaryTypesService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IUltimateBeneficiaryTypes> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((ultimateBeneficiaryTypes: HttpResponse<UltimateBeneficiaryTypes>) => {
    ...
    return of(new UltimateBeneficiaryTypes());
  }
}
