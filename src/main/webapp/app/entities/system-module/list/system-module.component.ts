...
export class SystemModuleComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.systemModuleService
        ...
        .subscribe(
          (res: HttpResponse<ISystemModule[]>) => {
  ...
  search(query: string): void {
    if (query && ['moduleName'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: ISystemModule): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ISystemModule[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
