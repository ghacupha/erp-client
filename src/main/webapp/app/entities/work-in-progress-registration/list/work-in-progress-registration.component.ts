...
import { EntityArrayResponseType, WorkInProgressRegistrationService } from '../service/work-in-progress-registration.service';
...
export class WorkInProgressRegistrationComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['sequenceNumber', 'particulars', 'comments'];
  ...
  trackId = (_index: number, item: IWorkInProgressRegistration): number =>
    this.workInProgressRegistrationService.getWorkInProgressRegistrationIdentifier(item);

  search(query: string): void {
    if (query && WorkInProgressRegistrationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (WorkInProgressRegistrationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.workInProgressRegistrations = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IWorkInProgressRegistration[] | null): IWorkInProgressRegistration[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.workInProgressRegistrationService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.workInProgressRegistrationService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
