...
export class CrbComplaintStatusTypeComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.crbComplaintStatusTypeService
        .search({
          page: pageToLoad - 1,
        ...
        .subscribe(
          (res: HttpResponse<ICrbComplaintStatusType[]>) => {
  ...
  search(query: string): void {
    if (query && ['complaintStatusTypeCode', 'complaintStatusType', 'complaintStatusDetails'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: ICrbComplaintStatusType): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ICrbComplaintStatusType[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/crb-complaint-status-type'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.crbComplaintStatusTypes = data ?? [];
    this.ngbPaginationPage = this.page;
  }
