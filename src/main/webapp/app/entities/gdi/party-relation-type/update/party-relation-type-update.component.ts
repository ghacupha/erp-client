...
export class PartyRelationTypeUpdateComponent implements OnInit {
  ...
  editForm = this.fb.group({
    id: [],
    partyRelationTypeCode: [null, [Validators.required]],
    partyRelationType: [null, [Validators.required]],
    partyRelationTypeDescription: [],
  });
  ...
  save(): void {
    this.isSaving = true;
    const partyRelationType = this.createFromForm();
    if (partyRelationType.id !== undefined) {
  ...
  protected updateForm(partyRelationType: IPartyRelationType): void {
    this.editForm.patchValue({
      id: partyRelationType.id,
      partyRelationTypeCode: partyRelationType.partyRelationTypeCode,
      partyRelationType: partyRelationType.partyRelationType,
      partyRelationTypeDescription: partyRelationType.partyRelationTypeDescription,
    });
  }

  protected createFromForm(): IPartyRelationType {
    return {
      ...new PartyRelationType(),
      id: this.editForm.get(['id'])!.value,
      partyRelationTypeCode: this.editForm.get(['partyRelationTypeCode'])!.value,
      partyRelationType: this.editForm.get(['partyRelationType'])!.value,
      partyRelationTypeDescription: this.editForm.get(['partyRelationTypeDescription'])!.value,
    };
  }
}
