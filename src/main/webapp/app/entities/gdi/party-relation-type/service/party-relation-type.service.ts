...
import { IPartyRelationType, getPartyRelationTypeIdentifier } from '../party-relation-type.model';
...
export class PartyRelationTypeService {
  ...
  create(partyRelationType: IPartyRelationType): Observable<EntityResponseType> {
  ...
  update(partyRelationType: IPartyRelationType): Observable<EntityResponseType> {
    return this.http.put<IPartyRelationType>(
      `${this.resourceUrl}/${getPartyRelationTypeIdentifier(partyRelationType) as number}`,
      partyRelationType,
      { observe: 'response' }
    );
  }

  partialUpdate(partyRelationType: IPartyRelationType): Observable<EntityResponseType> {
  ...
  addPartyRelationTypeToCollectionIfMissing(
    partyRelationTypeCollection: IPartyRelationType[],
    ...partyRelationTypesToCheck: (IPartyRelationType | null | undefined)[]
  ): IPartyRelationType[] {
    const partyRelationTypes: IPartyRelationType[] = partyRelationTypesToCheck.filter(isPresent);
    if (partyRelationTypes.length > 0) {
      const partyRelationTypeCollectionIdentifiers = partyRelationTypeCollection.map(
        partyRelationTypeItem => getPartyRelationTypeIdentifier(partyRelationTypeItem)!
      );
      const partyRelationTypesToAdd = partyRelationTypes.filter(partyRelationTypeItem => {
        const partyRelationTypeIdentifier = getPartyRelationTypeIdentifier(partyRelationTypeItem);
        if (partyRelationTypeIdentifier == null || partyRelationTypeCollectionIdentifiers.includes(partyRelationTypeIdentifier)) {
          return false;
        }
