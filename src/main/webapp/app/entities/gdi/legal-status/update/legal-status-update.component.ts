...
export class LegalStatusUpdateComponent implements OnInit {
  ...
  editForm = this.fb.group({
    id: [],
    legalStatusCode: [null, [Validators.required]],
    legalStatusType: [null, [Validators.required]],
  ...
  save(): void {
    this.isSaving = true;
    const legalStatus = this.createFromForm();
    if (legalStatus.id !== undefined) {
  ...
  protected updateForm(legalStatus: ILegalStatus): void {
    this.editForm.patchValue({
      id: legalStatus.id,
      legalStatusCode: legalStatus.legalStatusCode,
      legalStatusType: legalStatus.legalStatusType,
      legalStatusDescription: legalStatus.legalStatusDescription,
    });
  }

  protected createFromForm(): ILegalStatus {
    return {
      ...new LegalStatus(),
      id: this.editForm.get(['id'])!.value,
      legalStatusCode: this.editForm.get(['legalStatusCode'])!.value,
      legalStatusType: this.editForm.get(['legalStatusType'])!.value,
      legalStatusDescription: this.editForm.get(['legalStatusDescription'])!.value,
    };
  }
}
