...
export class RemittanceFlagRoutingResolveService implements Resolve<IRemittanceFlag> {
  constructor(protected service: RemittanceFlagService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IRemittanceFlag> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((remittanceFlag: HttpResponse<RemittanceFlag>) => {
