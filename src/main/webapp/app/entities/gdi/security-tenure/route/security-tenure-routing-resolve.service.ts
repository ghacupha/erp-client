...
export class SecurityTenureRoutingResolveService implements Resolve<ISecurityTenure> {
  constructor(protected service: SecurityTenureService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ISecurityTenure> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((securityTenure: HttpResponse<SecurityTenure>) => {
