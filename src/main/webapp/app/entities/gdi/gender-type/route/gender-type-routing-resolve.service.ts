...
export class GenderTypeRoutingResolveService implements Resolve<IGenderType> {
  constructor(protected service: GenderTypeService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IGenderType> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((genderType: HttpResponse<GenderType>) => {
          if (genderType.body) {
