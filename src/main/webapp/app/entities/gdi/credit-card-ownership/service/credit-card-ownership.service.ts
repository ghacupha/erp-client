...
import { ICreditCardOwnership, getCreditCardOwnershipIdentifier } from '../credit-card-ownership.model';
...
export class CreditCardOwnershipService {
  ...
  create(creditCardOwnership: ICreditCardOwnership): Observable<EntityResponseType> {
  ...
  update(creditCardOwnership: ICreditCardOwnership): Observable<EntityResponseType> {
    return this.http.put<ICreditCardOwnership>(
      `${this.resourceUrl}/${getCreditCardOwnershipIdentifier(creditCardOwnership) as number}`,
      creditCardOwnership,
      { observe: 'response' }
    );
  }

  partialUpdate(creditCardOwnership: ICreditCardOwnership): Observable<EntityResponseType> {
  ...
  addCreditCardOwnershipToCollectionIfMissing(
    creditCardOwnershipCollection: ICreditCardOwnership[],
    ...creditCardOwnershipsToCheck: (ICreditCardOwnership | null | undefined)[]
  ): ICreditCardOwnership[] {
    const creditCardOwnerships: ICreditCardOwnership[] = creditCardOwnershipsToCheck.filter(isPresent);
    if (creditCardOwnerships.length > 0) {
      const creditCardOwnershipCollectionIdentifiers = creditCardOwnershipCollection.map(
        creditCardOwnershipItem => getCreditCardOwnershipIdentifier(creditCardOwnershipItem)!
      );
      const creditCardOwnershipsToAdd = creditCardOwnerships.filter(creditCardOwnershipItem => {
        const creditCardOwnershipIdentifier = getCreditCardOwnershipIdentifier(creditCardOwnershipItem);
        if (creditCardOwnershipIdentifier == null || creditCardOwnershipCollectionIdentifiers.includes(creditCardOwnershipIdentifier)) {
          return false;
        }
