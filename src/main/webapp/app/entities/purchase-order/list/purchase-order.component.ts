...
export class PurchaseOrderComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPurchaseOrder): number => this.purchaseOrderService.getPurchaseOrderIdentifier(item);

  search(query: string): void {
    if (query && PurchaseOrderComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (PurchaseOrderComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.purchaseOrders = dataFromBody;
  }
