...
import { EntityArrayResponseType, CountyCodeService } from '../service/county-code.service';
...
export class CountyCodeComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['countyName', 'subCountyName'];
  ...
  trackId = (_index: number, item: ICountyCode): number => this.countyCodeService.getCountyCodeIdentifier(item);

  search(query: string): void {
    if (query && CountyCodeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (CountyCodeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.countyCodes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ICountyCode[] | null): ICountyCode[] {
    const countyCodesNew = this.countyCodes ?? [];
    if (data) {
      for (const d of data) {
        if (countyCodesNew.map(op => op.id).indexOf(d.id) === -1) {
          countyCodesNew.push(d);
        }
      }
    }
    return countyCodesNew;
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.countyCodeService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.countyCodeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
