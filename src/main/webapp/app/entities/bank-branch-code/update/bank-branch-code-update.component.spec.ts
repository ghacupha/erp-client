...
describe('BankBranchCode Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    ...
    it('Should update editForm', () => {
      const bankBranchCode: IBankBranchCode = { id: 456 };
      const placeholders: IPlaceholder = { id: 65138 };
      bankBranchCode.placeholders = [placeholders];

      activatedRoute.data = of({ bankBranchCode });
      comp.ngOnInit();

      expect(comp.editForm.value).toEqual(expect.objectContaining(bankBranchCode));
  ...
  describe('save', () => {
    it('Should call update service on save for existing entity', () => {
      // GIVEN
      const saveSubject = new Subject<HttpResponse<BankBranchCode>>();
      ...
      expect(bankBranchCodeService.update).toHaveBeenCalledWith(bankBranchCode);
    ...
    it('Should call create service on save for new entity', () => {
      // GIVEN
      const saveSubject = new Subject<HttpResponse<BankBranchCode>>();
      const bankBranchCode = new BankBranchCode();
      ...
      // THEN
      expect(bankBranchCodeService.create).toHaveBeenCalledWith(bankBranchCode);
      expect(comp.isSaving).toEqual(false);
      expect(comp.previousState).toHaveBeenCalled();
    });
