...
export class FiscalYearComponent implements OnInit {
  ...
  search(query: string): void {
    if (query && ['fiscalYearCode', 'fiscalYearStatus'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: IFiscalYear): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IFiscalYear[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/fiscal-year'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
