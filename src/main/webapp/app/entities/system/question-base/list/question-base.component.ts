...
import { EntityArrayResponseType, QuestionBaseService } from '../service/question-base.service';
import { QuestionBaseDeleteDialogComponent } from '../delete/question-base-delete-dialog.component';
import { FilterOptions, IFilterOptions, IFilterOption } from 'app/shared/filter/filter.model';

@Component({
  selector: 'jhi-question-base',
...
export class QuestionBaseComponent implements OnInit {
  ...
  trackId = (_index: number, item: IQuestionBase): number => this.questionBaseService.getQuestionBaseIdentifier(item);

  search(query: string): void {
    if (query && QuestionBaseComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (QuestionBaseComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.questionBases = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IQuestionBase[] | null): IQuestionBase[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.questionBaseService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.questionBaseService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
