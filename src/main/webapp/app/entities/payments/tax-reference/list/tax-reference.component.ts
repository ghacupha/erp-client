...
import { EntityArrayResponseType, TaxReferenceService } from '../service/tax-reference.service';
...
export class TaxReferenceComponent implements OnInit {
  ...
  trackId = (_index: number, item: ITaxReference): number => this.taxReferenceService.getTaxReferenceIdentifier(item);

  search(query: string): void {
    if (query && TaxReferenceComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (TaxReferenceComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.taxReferences = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ITaxReference[] | null): ITaxReference[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.taxReferenceService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.taxReferenceService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
