...
describe('FileType Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [RouterTestingModule.withRoutes([{ path: 'file-type', component: FileTypeComponent }]), HttpClientTestingModule],
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.fileTypes?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to fileTypeService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getFileTypeIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getFileTypeIdentifier).toHaveBeenCalledWith(entity);
