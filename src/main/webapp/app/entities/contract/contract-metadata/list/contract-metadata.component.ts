...
export class ContractMetadataComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.contractMetadataService
        ...
        .subscribe(
          (res: HttpResponse<IContractMetadata[]>) => {
  ...
  trackId(index: number, item: IContractMetadata): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IContractMetadata[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/contract-metadata'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
