...
import { EntityArrayResponseType, WorkInProgressTransferService } from '../service/work-in-progress-transfer.service';
...
export class WorkInProgressTransferComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['description', 'targetAssetNumber'];
  ...
  trackId = (_index: number, item: IWorkInProgressTransfer): number =>
    this.workInProgressTransferService.getWorkInProgressTransferIdentifier(item);

  search(query: string): void {
    if (query && WorkInProgressTransferComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (WorkInProgressTransferComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.workInProgressTransfers = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IWorkInProgressTransfer[] | null): IWorkInProgressTransfer[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.workInProgressTransferService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.workInProgressTransferService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
