...
import { EntityArrayResponseType, AssetRegistrationService } from '../service/asset-registration.service';
...
export class AssetRegistrationComponent implements OnInit {
  ...
  trackId = (_index: number, item: IAssetRegistration): number => this.assetRegistrationService.getAssetRegistrationIdentifier(item);

  search(query: string): void {
    if (query && AssetRegistrationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (AssetRegistrationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.assetRegistrations = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IAssetRegistration[] | null): IAssetRegistration[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.assetRegistrationService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.assetRegistrationService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
