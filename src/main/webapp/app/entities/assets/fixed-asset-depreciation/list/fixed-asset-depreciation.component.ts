...
import { EntityArrayResponseType, FixedAssetDepreciationService } from '../service/fixed-asset-depreciation.service';
...
export class FixedAssetDepreciationComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFixedAssetDepreciation): number =>
    this.fixedAssetDepreciationService.getFixedAssetDepreciationIdentifier(item);

  search(query: string): void {
    if (query && FixedAssetDepreciationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (FixedAssetDepreciationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.fixedAssetDepreciations = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFixedAssetDepreciation[] | null): IFixedAssetDepreciation[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.fixedAssetDepreciationService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.fixedAssetDepreciationService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
