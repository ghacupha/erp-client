...
export class DepreciationReportComponent implements OnInit {
  depreciationReports?: IDepreciationReport[];
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.depreciationReportService
        .search({
          page: pageToLoad - 1,
        ...
        .subscribe(
          (res: HttpResponse<IDepreciationReport[]>) => {
  ...
  delete(depreciationReport: IDepreciationReport): void {
    const modalRef = this.modalService.open(DepreciationReportDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.depreciationReport = depreciationReport;
  ...
  protected onSuccess(data: IDepreciationReport[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/depreciation-report'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
