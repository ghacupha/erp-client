...
export class ReportTemplateComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.reportTemplateService
        ...
        .subscribe(
          (res: HttpResponse<IReportTemplate[]>) => {
  ...
  search(query: string): void {
    if (query && ['catalogueNumber', 'description', 'notes', 'reportFile', 'compileReportFile'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: IReportTemplate): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IReportTemplate[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/report-template'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
