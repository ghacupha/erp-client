...
export class LeasePaymentComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.leasePaymentService
        ...
        .subscribe(
          (res: HttpResponse<ILeasePayment[]>) => {
  ...
  delete(leasePayment: ILeasePayment): void {
    const modalRef = this.modalService.open(LeasePaymentDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.leasePayment = leasePayment;
  ...
  protected onSuccess(data: ILeasePayment[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/lease-payment'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
