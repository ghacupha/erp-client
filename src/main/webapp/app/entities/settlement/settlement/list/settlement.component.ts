...
import { EntityArrayResponseType, SettlementService } from '../service/settlement.service';
...
export class SettlementComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISettlement): number => this.settlementService.getSettlementIdentifier(item);

  search(query: string): void {
    if (query && SettlementComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (SettlementComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.settlements = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISettlement[] | null): ISettlement[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.settlementService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.settlementService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
