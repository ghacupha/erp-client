...
import { EntityArrayResponseType, PaymentCategoryService } from '../service/payment-category.service';
...
export class PaymentCategoryComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPaymentCategory): number => this.paymentCategoryService.getPaymentCategoryIdentifier(item);

  search(query: string): void {
    if (query && PaymentCategoryComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (PaymentCategoryComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.paymentCategories = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IPaymentCategory[] | null): IPaymentCategory[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.paymentCategoryService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.paymentCategoryService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
