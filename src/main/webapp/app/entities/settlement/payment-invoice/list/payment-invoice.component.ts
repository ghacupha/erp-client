...
import { EntityArrayResponseType, PaymentInvoiceService } from '../service/payment-invoice.service';
...
export class PaymentInvoiceComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['invoiceNumber', 'fileUploadToken', 'compilationToken', 'remarks'];
  ...
  trackId = (_index: number, item: IPaymentInvoice): number => this.paymentInvoiceService.getPaymentInvoiceIdentifier(item);

  search(query: string): void {
    if (query && PaymentInvoiceComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (PaymentInvoiceComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.paymentInvoices = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IPaymentInvoice[] | null): IPaymentInvoice[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.paymentInvoiceService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.paymentInvoiceService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
