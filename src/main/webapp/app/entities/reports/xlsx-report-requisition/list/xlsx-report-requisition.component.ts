...
import { EntityArrayResponseType, XlsxReportRequisitionService } from '../service/xlsx-report-requisition.service';
...
export class XlsxReportRequisitionComponent implements OnInit {
  ...
  trackId = (_index: number, item: IXlsxReportRequisition): number =>
    this.xlsxReportRequisitionService.getXlsxReportRequisitionIdentifier(item);

  search(query: string): void {
    if (query && XlsxReportRequisitionComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (XlsxReportRequisitionComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.xlsxReportRequisitions = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IXlsxReportRequisition[] | null): IXlsxReportRequisition[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.xlsxReportRequisitionService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.xlsxReportRequisitionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
