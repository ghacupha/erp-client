...
import { EntityArrayResponseType, AgencyNoticeService } from '../service/agency-notice.service';
...
export class AgencyNoticeComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['referenceNumber', 'agencyStatus', 'assessmentNotice'];
  ...
  trackId = (_index: number, item: IAgencyNotice): number => this.agencyNoticeService.getAgencyNoticeIdentifier(item);

  search(query: string): void {
    if (query && AgencyNoticeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (AgencyNoticeComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.agencyNotices = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IAgencyNotice[] | null): IAgencyNotice[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.agencyNoticeService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.agencyNoticeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
