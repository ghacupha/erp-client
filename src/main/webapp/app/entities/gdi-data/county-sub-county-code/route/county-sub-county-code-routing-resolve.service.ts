...
export class CountySubCountyCodeRoutingResolveService implements Resolve<ICountySubCountyCode> {
  constructor(protected service: CountySubCountyCodeService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ICountySubCountyCode> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((countySubCountyCode: HttpResponse<CountySubCountyCode>) => {
    ...
    return of(new CountySubCountyCode());
  }
}
