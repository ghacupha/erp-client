...
export class WeeklyCashHoldingRoutingResolveService implements Resolve<IWeeklyCashHolding> {
  constructor(protected service: WeeklyCashHoldingService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IWeeklyCashHolding> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((weeklyCashHolding: HttpResponse<WeeklyCashHolding>) => {
    ...
    return of(new WeeklyCashHolding());
  }
}
