...
export class SettlementRequisitionComponent implements OnInit {
  ...
  search(query: string): void {
    if (query && ['description', 'serialNumber', 'requisitionNumber', 'paymentStatus', 'transactionId'].includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
  ...
  trackId(index: number, item: ISettlementRequisition): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ISettlementRequisition[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/settlement-requisition'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.settlementRequisitions = data ?? [];
    this.ngbPaginationPage = this.page;
  }
