...
import { EntityArrayResponseType, OutletStatusService } from '../service/outlet-status.service';
...
export class OutletStatusComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['branchStatusTypeCode', 'branchStatusType', 'branchStatusTypeDescription'];
  ...
  trackId = (_index: number, item: IOutletStatus): number => this.outletStatusService.getOutletStatusIdentifier(item);

  search(query: string): void {
    if (query && OutletStatusComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (OutletStatusComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.outletStatuses = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IOutletStatus[] | null): IOutletStatus[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.outletStatusService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.outletStatusService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
