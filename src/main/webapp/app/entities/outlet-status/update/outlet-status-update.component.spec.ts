...
describe('OutletStatus Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    ...
    it('Should update editForm', () => {
      const outletStatus: IOutletStatus = { id: 456 };
      const placeholders: IPlaceholder = { id: 90519 };
      outletStatus.placeholders = [placeholders];

      activatedRoute.data = of({ outletStatus });
      comp.ngOnInit();

      expect(comp.editForm.value).toEqual(expect.objectContaining(outletStatus));
  ...
  describe('save', () => {
    it('Should call update service on save for existing entity', () => {
      // GIVEN
      const saveSubject = new Subject<HttpResponse<OutletStatus>>();
      ...
      expect(outletStatusService.update).toHaveBeenCalledWith(outletStatus);
    ...
    it('Should call create service on save for new entity', () => {
      // GIVEN
      const saveSubject = new Subject<HttpResponse<OutletStatus>>();
      const outletStatus = new OutletStatus();
      ...
      // THEN
      expect(outletStatusService.create).toHaveBeenCalledWith(outletStatus);
      expect(comp.isSaving).toEqual(false);
      expect(comp.previousState).toHaveBeenCalled();
    });
