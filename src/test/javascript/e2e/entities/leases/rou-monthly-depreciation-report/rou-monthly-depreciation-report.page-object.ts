...
export class RouMonthlyDepreciationReportComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-rou-monthly-depreciation-report div table .btn-danger'));
  title = element.all(by.css('jhi-rou-monthly-depreciation-report div h2#page-heading span')).first();
...
export class RouMonthlyDepreciationReportUpdatePage {
  pageTitle = element(by.id('jhi-rou-monthly-depreciation-report-heading'));
  ...
  reportingYearSelect = element(by.id('field_reportingYear'));
  ...
  async reportingYearSelectLastOption(): Promise<void> {
    await this.reportingYearSelect.all(by.tagName('option')).last().click();
  }

  async reportingYearSelectOption(option: string): Promise<void> {
    await this.reportingYearSelect.sendKeys(option);
  }

  getReportingYearSelect(): ElementFinder {
    return this.reportingYearSelect;
  }

  async getReportingYearSelectedOption(): Promise<string> {
    return await this.reportingYearSelect.element(by.css('option:checked')).getText();
  }
...
export class RouMonthlyDepreciationReportDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-rouMonthlyDepreciationReport-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-rouMonthlyDepreciationReport'));
